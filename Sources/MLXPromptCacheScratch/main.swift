...
let store = try PromptCacheStore(directory: dir, budgetBytes: 8_000_000_000, signature: signature, blockSize: blockSize)
...
================ RESULT ================
output identical : \(cold.text == warm.text ? "✅ YES" : "❌ NO")
...
ready : \(String(format: "%.0f", coldReady)) ms → \(String(format: "%.0f", warmReady)) ms  (\(String(format: "%.1fx", coldReady / max(warmReady, 0.001))) faster)
========================================
""")
if cold.text != warm.text { print("\nCOLD:\n\(cold.text)\n\nWARM:\n\(warm.text)") }
